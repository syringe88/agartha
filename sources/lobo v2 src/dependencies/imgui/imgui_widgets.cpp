...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool underneath_checkbox )
{
	...
	frame_animation.Update( pressed || held ? 4.f : -4.f, 1.f, 0.f, 1.f * g_menu.m_animation_progress );
	...
	RenderFrame( bb.Min - ImVec2( 2.f, 2.f ), bb.Max + ImVec2( 2.f, 2.f ),
	             ImColor( 25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f * g_menu.m_animation_progress ), true,
	             g.Style.FrameRounding );
	RenderFrame( bb.Min - ImVec2( 2.f, 2.f ), bb.Max + ImVec2( 2.f, 2.f ),
	             ImColor( accent_color.Value.x, accent_color.Value.y, accent_color.Value.z,
	                      1.f * frame_animation.AnimationData->second * g_menu.m_animation_progress ),
	             true,
	             g.Style.FrameRounding );
	window->DrawList->AddRect( bb.Min - ImVec2( 2.f, 2.f ), bb.Max + ImVec2( 2.f, 2.f ),
	                           ImColor( 50, 50, 50, int( 100 * g_menu.m_animation_progress ) ),
	                           g.Style.FrameRounding );
	...
	text_animation.Update( hovered || g.ActiveId == id ? 4.f : -4.f, 1.f, 0.5f, 1.f * g_menu.m_animation_progress );
	...
	PushStyleColor( ImGuiCol_Text, ImVec4( text_color.Value.x, text_color.Value.y, text_color.Value.z,
	                                       text_color.Value.w * text_animation.AnimationData->second * g_menu.m_animation_progress ) );
...
bool ImGui::Checkbox( const char* label, bool* v )
{
	...
	frame_animation.Update( *v ? 5.f : -5.f, 1.f, 0.f, 1.f * g_menu.m_animation_progress );
	...
	if ( render_decorations )
		RenderFrame( check_bb.Min, check_bb.Max,
		             ImColor( 25 / 255.f, 25 / 255.f, 25 / 255.f * g_menu.m_animation_progress, 1.f * g_menu.m_animation_progress ),
	...
	window->DrawList->AddRect( check_bb.Min, check_bb.Max, ImColor( 50, 50, 50, int( 100 * g_menu.m_animation_progress ) ), style.FrameRounding );

	auto text_animation = ImAnimationHelper( id + 1u, ImGui::GetIO( ).DeltaTime );
	text_animation.Update( hovered || *v ? 4.f : -4.f, 1.f, 0.5f, 1.f * g_menu.m_animation_progress );
...
void ImGui::ProgressBar( float fraction, const ImVec2& size_arg, const char* overlay )
{
	...
	RenderFrame( bb.Min, bb.Max, ImColor( 0,0,0,50), true, style.FrameRounding );
	bb.Expand( ImVec2( -style.FrameBorderSize, -style.FrameBorderSize ) );
	const ImVec2 fill_br = ImVec2( ImLerp( bb.Min.x, bb.Max.x, fraction ), bb.Max.y );
	RenderRectFilledRangeH( window->DrawList, bb, GetColorU32( ImGuiCol_Accent  ), 0.0f, fraction,
	                        style.FrameRounding );
...
void ImGui::OptionPopup( const char* str_id, const std::function< void( ) >& function, const ImVec2& window_size )
{
	...
	[ & ]( ) {
		...
		GetWindowDrawList( )->AddText(
			...
			hovered ? IM_COL32_WHITE : IM_COL32( 190, 190, 190, int( 255 * g_menu.m_animation_progress ) ), cog_icon );
	...
	if ( BeginPopup( str_id ) ) {
		...
		draw_list->AddRectFilled( ImVec2( position ), ImVec2( position.x + size.x, position.y + 20.f ),
		                          ImColor( 25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f * g_menu.m_animation_progress ),
		                          g.Style.WindowRounding - 2.f, ImDrawFlags_RoundCornersTop );

		PopClipRect( );

		PushClipRect( position, position + size, false );
		draw_list->AddRect( position, position + size, ImColor( 50, 50, 50, int( 100 * g_menu.m_animation_progress ) ),
		                    g.Style.WindowRounding - 2.f );
		...
		draw_list->AddText(
			...
			position + ImVec2( ( size.x - text_size.x ) / 2.f, ( 20.f - text_size.y ) / 2.f ),
		                    ImColor( text_color.Value.x, text_color.Value.y, text_color.Value.z,
		                             text_color.Value.w * text_animation.AnimationData->second * g_menu.m_animation_progress ),
			str_id );

		RenderFadedGradientLine( draw_list, position + ImVec2( 0.f, 20.f ), ImVec2( size.x, 1.f ),
		                         ImColor( accent_color.Value.x, accent_color.Value.y, accent_color.Value.z, 1.f * g_menu.m_animation_progress ) );
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags, bool underneath_checkbox )
{
	...
	if ( !( flags & ImGuiComboFlags_NoPreview ) ) {
		window->DrawList->AddRectFilled( frame_bb.Min, ImVec2( value_x2, frame_bb.Max.y ),
		                                 ImColor( 25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f * g_menu.m_animation_progress ),
		                                 style.PopupRounding );
		window->DrawList->AddRect( frame_bb.Min, ImVec2( value_x2, frame_bb.Max.y ), ImColor( 50, 50, 50, int( 100 * g_menu.m_animation_progress ) ),
		                           style.FrameRounding );
	}
	...
	if ( !( flags & ImGuiComboFlags_NoArrowButton ) ) {
		...
		RenderArrow( arrow_draw_position,
		             ImColor( accent_color.Value.x, accent_color.Value.y, accent_color.Value.z, 1.f * g_menu.m_animation_progress ) );
	}

	auto text_animation = ImAnimationHelper( id + ImHashStr( "text-animation" ), ImGui::GetIO( ).DeltaTime );
	text_animation.Update( hovered || popup_open ? 4.f : -4.f, 1.f, 0.5f, 1.f * g_menu.m_animation_progress );
	...
	PushStyleColor( ImGuiCol_::ImGuiCol_PopupBg, ImVec4( 25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f * g_menu.m_animation_progress ) );
	bool ret = Begin( name, NULL, window_flags );

	PushClipRect( GetWindowPos( ), GetWindowSize( ) + GetWindowPos( ), false );
	GetWindowDrawList( )->AddRect( GetWindowPos( ), GetWindowSize( ) + GetWindowPos( ),
	                               ImColor( 50, 50, 50, int( 100 * g_menu.m_animation_progress ) ),
	                               style.FrameRounding );
	PopClipRect( );
                          ...
                          ImGuiSliderFlags flags, bool underneath_checkbox )
{
	...
	RenderFrame( frame_bb.Min, frame_bb.Max, ImColor( 25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f * g_menu.m_animation_progress ), true,
	             g.Style.FrameRounding );

	window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 50, 50, 50, int( 100 * g_menu.m_animation_progress ) ), g.Style.FrameRounding );
                         ...
                         ImGuiInputTextCallback callback, void* callback_user_data, int width )
{
	...
	IM_ASSERT( !( ( flags & ImGuiInputTextFlags_CallbackHistory ) && ( flags & ImGuiInputTextFlags_Multiline ) ) );        // Can't use both together
	IM_ASSERT( !( ( flags & ImGuiInputTextFlags_CallbackCompletion ) && ( flags & ImGuiInputTextFlags_AllowTabInput ) ) ); // Can't use both together
	...
	if ( is_multiline ) // Open group before calling GetID()
		BeginGroup( );
	const ImGuiID id        = window->GetID( label );
	const ImVec2 label_size = CalcTextSize( label, NULL, true );
	const ImVec2 frame_size =
		CalcItemSize( size_arg, CalcItemWidth( ), ( is_multiline ? g.FontSize * 8.0f : label_size.y ) + style.FramePadding.y * 2.0f );
	...
	if ( render_cursor || render_selection ) {
		...
		// Отображаем метку слева сверху
		if ( label_size.x > 0 ) {
			ImVec2 label_pos = ImVec2( frame_bb.Min.x, frame_bb.Min.y - g.Font->FontSize - 3.0f );
			RenderText( label_pos, label );
		}

		// Отображаем подсказку "type here..." если буфер пуст
		if ( !is_multiline && buf[ 0 ] == 0 ) {
			const char* placeholder_text = "type here...";
			ImVec2 placeholder_pos       = draw_pos;

			// Розовый контур
			ImU32 outline_color  = GET_VARIABLE( g_variables.m_accent, c_color ).get_u32( float( 1.f * g_menu.m_animation_progress ) );
			float outline_offset = 1.0f;
			
			// Белый текст
			ImU32 text_color = IM_COL32( 255, 255, 255, int( 255 * g_menu.m_animation_progress ) );
			draw_window->DrawList->AddText( g.Font, g.FontSize, placeholder_pos, text_color, placeholder_text );
		}
	} else {
		if ( is_multiline )
		...
		if ( !is_multiline && buf[ 0 ] == 0 ) {
			...
			// Белый текст
			ImU32 text_color = GET_VARIABLE( g_variables.m_accent, c_color ).get_u32( float( 1.f * g_menu.m_animation_progress ) );
			draw_window->DrawList->AddText( g.Font, g.FontSize, placeholder_pos, text_color, placeholder_text );
		}
	}
...
bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg )
{
	...
	selected_text_animation.Update( selected ? 4.f : -4.f, 1.f, 0.f, 1.f * g_menu.m_animation_progress );

	auto hovered_text_animation = ImAnimationHelper( id + 1u, ImGui::GetIO( ).DeltaTime );
	hovered_text_animation.Update( hovered ? 4.f : -4.f, 1.f, 0.5f, 1.f * g_menu.m_animation_progress );

	const ImColor text_color = GetColorU32( ImGuiCol_Text );

	const ImColor accent_color = ImGui::GetColorU32( ImGuiCol_::ImGuiCol_Accent );

	const ImVec4 blended_color = ImColor::Blend( ImColor( text_color.Value.x, text_color.Value.y, text_color.Value.z,
	                             text_color.Value.w * hovered_text_animation.AnimationData->second * g_menu.m_animation_progress ),
		ImColor( accent_color.Value.x, accent_color.Value.y, accent_color.Value.z ),
	                    selected_text_animation.AnimationData->second * g_menu.m_animation_progress );
