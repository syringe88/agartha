...
public:
	...
	MaterialVarType_t get_type() {
		return (MaterialVarType_t)(*(unsigned char*)((uintptr_t)this + 0x1C) & 0x0F);
	}

	void set_vector_component(float* value) {
		set_vec_component_value(value[0], 0);
		set_vec_component_value(value[1], 1);
		set_vec_component_value(value[2], 2);
		set_vec_component_value(value[3], 3);
	}
};
