...
namespace math {
    ...
    #define M_PI2		3.14159265358979323846 * 2
	...
	inline vec3_t lerp_vec(const vec3_t& a, const vec3_t& b, float t)
	{
		return vec3_t(
			a.x + (b.x - a.x) * t,
			a.y + (b.y - a.y) * t,
	...
	__forceinline float vector_to_yaw(const vec3_t& dir) {
	...
	__forceinline float angle_diff(float dest_angle, float src_angle) {
	...
	{
		// clamps pitch to the normal Source engine limits
		while (pitch > 89.f)
			pitch -= 180.f;
		while (pitch < -89.f)
			pitch += 180.f;
