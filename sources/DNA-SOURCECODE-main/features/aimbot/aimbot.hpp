...
namespace aimbot {
    ...
    inline bool is_slowwalk = false;


    bool check_accuracy(c_usercmd* cmd);
    void prestop(c_usercmd* cmd);
...
namespace autowall {
    ...
    void ScanForMaxDamage(player_t* target, vec3_t src_position, weapon_info_t* weapon_data, fire_bullet_data& bullet_data);
}
...
public:
	...
	int selected_tick = 0;
	int sel_ent_index = 0;
	std::map<int, std::deque<bt_data::backtrack_data>> data;
};
