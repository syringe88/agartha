...
{
    i_net_channel_info* net_info = interfaces::engine->get_net_channel_info();
    static convar* sv_maxunlag = interfaces::console->get_convar("sv_maxunlag");

    if (!interfaces::engine->is_in_game() || !c::backtrack::backtrack || !c::backtrack::fake_latency ||
        datagram != nullptr || !net_info || !sv_maxunlag)
    {
    ...
    const float max_latency = std::max<float>(0.f, std::clamp(c::backtrack::fake, 0.f, sv_maxunlag->get_float()) - net_info->get_latency(FLOW_OUTGOING));
