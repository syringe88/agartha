...
#include "../includes/imgui/animations.h"
...
//    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(menu::menu_accent[0], menu::menu_accent[1], menu::menu_accent[2], it_anim->second.animation), false, 9.0f);
//
...
bool ImGui::Checkbox(const char* label, bool* v) {
    ...
    auto name_size = CalcTextSize(label, 0, 1);

    auto draw = window->DrawList;
    auto pos = window->DC.CursorPos;

    float square_sz = 18.f;
    ImRect bb(pos, { pos.x + square_sz, pos.y + square_sz });
    ImRect total_bb(pos, { pos.x + square_sz + name_size.x + GImGui->Style.ItemInnerSpacing.x, pos.y + square_sz });
    ItemAdd(total_bb, id);
    ItemSize(total_bb);
    ...
    struct s {
        ...
        float hover;
        ImColor col;
    }; auto& obj = anim_obj(label, 0, s{ 0.f, 0.f, 0 });

    obj.anim = animate(obj.anim, 0.f, 1.f, *v);
    obj.hover = animate(obj.hover, 0.f, 1.f, hovered && !(*v));
    obj.col = col_anim(GetColorU32(ImGuiCol_FrameBg), GetColorU32(ImGuiCol_FrameBgHovered), obj.hover);

    draw->AddText({ total_bb.Max.x - name_size.x, bb.GetCenter().y - name_size.y / 2 }, GetColorU32(ImGuiCol_Text), label, FindRenderedTextEnd(label));
    draw->AddRectFilled(bb.Min, bb.Max, obj.col, style.FrameRounding);
    draw->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), style.FrameRounding);
    draw->AddRectFilled(bb.Min, bb.Max, ImColor(menu::menu_accent[0], menu::menu_accent[1], menu::menu_accent[2], obj.anim), style.FrameRounding);
    RenderCheckMark(draw, { bb.GetCenter().x - (9 * obj.anim) / 2, bb.GetCenter().y - (9 * obj.anim) / 2 }, GetColorU32(ImGuiCol_FrameBg, obj.anim), 9 * obj.anim); 
...
static float ImGui::CalcMaxPopupHeightFromItemCount(int items_count)
{
    ImGuiContext& g = *GImGui;
    if (items_count <= 0)
        return FLT_MAX;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // Blood theme colors
    ImColor blood_accent = ImColor(10, 10, 12, 255);        // Deep red, low alpha
    ImColor hover_bg = ImColor(15, 15, 17, 255);        // Hover red
    ImColor active_bg = ImColor(30, 30, 30, 255);       // Optional active red

    ImColor bg_color = ImColor(10, 10, 10, 255);         // Background (dark blood red)
    ImColor bg_dark = ImColor(5, 5, 5, 255);           // Slightly darker bg
    ImColor text_dark = ImColor(23, 23, 23, 255);     // Pale bone text
    ImColor border_color = ImColor(20, 20, 20, 255);        // Desaturated red border
    ImColor disabled_text = ImColor(30, 30, 30, 255);         // Muted reddish gray
    ImColor popup_bg = ImColor(5, 5, 5, 255);          // Fully opaque dark popup
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, bg_color, 4.f);
    if (hovered)
        window->DrawList->AddRectFilled(clickable.Min, clickable.Max, hover_bg, 4.f);
    window->DrawList->AddRect(clickable.Min, clickable.Max, border_color, 4.f);

    // Label
    window->DrawList->AddText(rect.Min, text_dark, label);

    // Preview value text
    RenderTextClipped(clickable.Min + ImVec2(14, 7), clickable.Max - ImVec2(24, -47), preview_value, nullptr, nullptr, ImVec2(0.0f, 0.0f));

    // Arrow icon
    PushFont(combo_arrow);
    window->DrawList->AddText(
        ImVec2(clickable.Max.x - 20, (clickable.Min.y + clickable.Max.y) / 2 - arrow_size.y / 2),
        ImColor(blood_accent.Value.x, blood_accent.Value.y, blood_accent.Value.z, 0.2f + it_anim->second.arrow_anim),
        "A"
    );
    PopFont();
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(
            ImVec2(w, 0.0f),
            ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * it_anim->second.open_anim)
        );
    }
    ...
    PushStyleColor(ImGuiCol_Border, ImVec4(blood_accent.Value.x, blood_accent.Value.y, blood_accent.Value.z, 1.f));
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(blood_accent.Value.x, blood_accent.Value.y, blood_accent.Value.z, 1.f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Animation state
    struct slider_element { float opacity; };
    static std::map<ImGuiID, slider_element> anim;
    auto& a = anim.try_emplace(id, slider_element{ 0.f }).first->second;
    a.opacity = ImLerp(a.opacity, IsItemActive() ? 1.0f : 0.4f, 0.08f * (1.0f - g.IO.DeltaTime));
    ...
    ImColor bg = ImColor(15, 15, 15, 60);
    ImColor fill = ImColor(menu::menu_accent[0], menu::menu_accent[1], menu::menu_accent[2], 130.f);
    ImColor grab = ImColor(menu::menu_accent[0], menu::menu_accent[1], menu::menu_accent[2], float(255.f * a.opacity));
    ImColor text = ImColor(242, 242, 242, int(255 * a.opacity));

    // Make the frame shorter
    float frame_height = 3.0f; // desired slider track height
    ImVec2 frame_top_left = ImVec2(frame_bb.Min.x, frame_bb.Min.y + (frame_bb.GetHeight() - frame_height) * 0.5f);
    ImVec2 frame_bottom_right = ImVec2(frame_bb.Max.x, frame_top_left.y + frame_height);
    ImRect short_frame_bb(frame_top_left, frame_bottom_right);

    // Draw the shortened frame
    window->DrawList->AddRectFilled(short_frame_bb.Min, short_frame_bb.Max, bg, 0.0f);

    // Draw the filled portion of slider
    window->DrawList->AddRectFilled(
        ImVec2(short_frame_bb.Min.x, short_frame_bb.Min.y),
        ImVec2(grab_bb.Min.x + 3, short_frame_bb.Max.y),
        fill,
        0.0f
    );

    // Draw grab handle (circle)
   // Draw grab handle (rectangle instead of circle)
   // Draw grab handle (taller rectangle)
    float grab_width = 20.0f;
    float grab_height = frame_bb.GetHeight() * 1.5f; // 1.5x taller than the slider
    float grab_top = frame_bb.Min.y - (grab_height - frame_bb.GetHeight()) * 0.5f; // center over track
    float grab_bottom = grab_top + grab_height;

    window->DrawList->AddRectFilled(
        ImVec2(grab_bb.Min.x - grab_width * 0.5f, grab_top),
        ImVec2(grab_bb.Min.x + grab_width * 0.5f, grab_bottom),
        grab,
        2.0f // rounding, optional
    );

    // Draw label and value
    RenderText(total_bb.Min, label);
    PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.f, 1.f, a.opacity));
    {
...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ...
    int key_width = ImGui::CalcTextSize(keys[*current_key]).x * 1.5f;

    // Fixed positioning - move further right and prevent shifting
    const float fixed_offset = 45.f; // Increased to move keybind more to the right
    SameLine(group_w - fixed_offset);
    ImGui::SetCursorPosX(group_w - fixed_offset);
    ...
    if (g.ActiveId == id) {
        ...
        if (key_style) {
            ...
            if (popup_open) {
                SetNextWindowSize(ImVec2(85, CalcMaxPopupHeightFromItemCount(3)));
    ...
    window->DrawList->AddText(frame_bb.Min, g.ActiveId == id ? ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, g.Style.Alpha) : ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, g.Style.Alpha), buf_display);
    PopFont();
