...
void sequence_proxy( const c_recv_proxy_data* p_data, void* p_struct, void* p_out ) {
	const auto local = static_cast< player_t* >( interfaces::ent_list->get_client_entity( interfaces::engine->get_local_player( ) ) );

	if (!local || !local->is_alive( ))
		return ofunc( p_data, p_struct, p_out );

	const auto proxy_data = const_cast< c_recv_proxy_data* >( p_data );
	const auto view_model = static_cast< base_view_model* >( p_struct );

	if (view_model && view_model->m_howner( ) && view_model->m_howner( ) != NULL) {
		const auto owner = static_cast< player_t* >( interfaces::ent_list->get_client_entity_handle( view_model->m_howner( ) ) );
		...
		if (!strcmp( client_class->network_name, "CBaseViewModel" )) {
			...
			for (int nIndex = 0; nIndex < table->props_count; nIndex++) {
				...
				if (!pProp || strcmp( pProp->prop_name, "m_nSequence" ))
					continue;
				ofunc = static_cast< recv_var_proxy_fn >( pProp->proxy_fn );
...
void features::skins::init_knife_proxy( ) {
	knife_sequence( );
}